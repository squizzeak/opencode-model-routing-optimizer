...
Config files don't record whether a provider is a subscription or pay-per-token (the same provider key can back either). When inference is needed, ask once via `pick_many` over the configured providers: which are subscription/bundled, which pay-per-token, which free. Then: **cheapest-bundled** = lowest effective $/1M among the user's subscriptions (ask if unknown — never guess fees); **largest-subscription** = most quota headroom + strongest models for `@heavy`. Surface the inferred pair for confirmation.
...
3. Record provenance: what was verified live today vs. user-asserted — put it in the JSONC comments and the final summary.
