...
2. **Live quota telemetry (recommended)**: check for `opencode-quota` and run `opencode-quota show --json` when available. If missing, offer `npm install -g @slkiser/opencode-quota@latest` after checking Node ≥ 22; never run `opencode-quota init` from this command. If the user declines, continue with quota claims labeled **user-asserted**, not verified-live.
