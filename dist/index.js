...
async function installAll(assetsDir, configRoot, client) {
    const report = { installed: [], skipped: [], failed: [] };
    for (const target of INSTALL_TARGETS) {
        try {
            const result = await installOne(assetsDir, configRoot, target, PLUGIN_VERSION);
            if (result.status === "installed") {
                ...
                await log(client, "warn", `${target.srcRel} is missing the routing-optimizer version marker — refusing to install. Reinstall the plugin.`);
            }
        }
        catch (err) {
...
export const RoutingOptimizerPlugin = async ({ client }) => {
