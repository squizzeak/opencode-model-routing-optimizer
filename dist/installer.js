...
import { readFile, writeFile, mkdir, access, open, lstat, rmdir, unlink, constants, } from "node:fs/promises";
 ...
 */
export const RETIRED_ASSET_HASHES = {
 ...
 */
export const RETIRED_TARGETS = Object.entries(RETIRED_ASSET_HASHES).map(([dstRel, knownHashes]) => {
 ...
 */
export async function retireOne(absConfigRoot, target) {
    ...
    if (!target.knownHashes.includes(hash)) {
        return {
            status: "preserved",
            target,
            reason: "content differs from every shipped revision — left untouched (may be user-edited)",
        };
    }
 ...
 */
export async function readVersion(filePath) {
    let fd;
    try {
 ...
 */
export function needsInstall(srcVersion, dstVersion) {
    // Destination missing → install
    if (dstVersion === undefined)
        return true;
 ...
 */
export async function installOne(absPluginAssetsDir, absConfigRoot, target, pluginVersion) {
